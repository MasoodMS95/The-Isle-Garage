...
Readable-link regression checks cover one-time handle selection, case normalization, duplicate/concurrent requests, reserved/invalid names, migration preservation of visibility and IDs, canonical metadata, old/provisional redirects, and page/API/OG denial when private. Browser QA must verify 390px dialog bounds, 44px link/copy controls, normal clipboard paste and the denied/unavailable clipboard selection fallback; a successful API promise alone cannot independently prove the operating-system clipboard contents.
